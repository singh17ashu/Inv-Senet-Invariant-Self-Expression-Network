...
class DataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train, batch_size=self.batch_size, shuffle=True, num_workers=16, pin_memory=True)
...
class Engine(pl.LightningModule):
    ...
    def training_step(self, batch_full, batch_idx):
        ...
        _ , pseudo_pred_color = self.model_bias(k_batch)
        ...
        k_batch = self.model_cluster.key_embedding(batch)
        k_batch_color = self.grad_reverse(k_batch)
        pred_color , _ = self.model_bias(k_batch_color)
       
        loss_color = F.cross_entropy(pred_color, labels)
...
def main():
    ...
    if not args.eval:
        ...
        if not args.feat:
            ...
            csv_logger = pl_loggers.CSVLogger("logs_st_hyp", name=args.dataset + '_'+ args.feat_type)
            ...
            if args.resume is not None:
                ...
                trainer = Trainer(gpus=1, strategy='dp', max_epochs=n_epochs, logger=csv_logger, callbacks=[checkpoint_callback], profiler="simple")
                trainer.fit(pl_model, data_module)
