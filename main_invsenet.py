...
class Engine(pl.LightningModule):
    ...
    def training_step(self, batch_full, batch_idx):
        ...
        sch_cluster, sch_bias = self.lr_schedulers()
        ...
        nn.utils.clip_grad_norm_(self.model_bias.parameters(), 0.001)
        opt_cluster.step()
    ...
    def configure_optimizers(self):
        opt_cluster = torch.optim.AdamW(self.model_cluster.parameters(), lr=self.learning_rate_cluster)
        opt_bias = torch.optim.AdamW(self.model_bias.parameters(), lr=self.learning_rate_bias)
        sch_cluster = optim.lr_scheduler.CosineAnnealingLR(opt_cluster, T_max=self.n_epochs, eta_min=self.lr_min)
        sch_bias = optim.lr_scheduler.CosineAnnealingLR(opt_bias, T_max=self.n_epochs, eta_min=self.lr_min)
        return ([opt_cluster, opt_bias],[sch_cluster, sch_bias])
...
def main():
    ...
    if not args.eval:
        ...
        csv_logger = pl_loggers.CSVLogger("logs_invsenet_v2", name=args.dataset + '_'+ args.feat_type)
